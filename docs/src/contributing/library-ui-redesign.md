...
- **Cover does not get covered.** `MenuItemCover.genCover` is called with `card_width × cover_height`, where `cover_height = card_height − text_band_h`. Real covers render with **no border** — the art itself is the visual boundary; an unconditional black frame around every cover read as cluttered on device. Empty (no-cover) cells keep a faint `COLOR_GRAY_9` outline so they're still distinguishable from the page background.
- **Dynamic text band.** `text_band_h` is computed at runtime from the actual title + timestamp `TextWidget` heights — a fixed 36px reserve underestimated typical font sizes and the timestamp got clipped by the next row. Title is `cfont` bold; timestamp drops to `infont_size − 2`. Single line each, ellipsized at `card_width − 2 × 4px` padding. Title-only entries (no `mandatory`) shrink the band further, giving the cover the freed pixels back.
